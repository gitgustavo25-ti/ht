...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['role']) && !empty($_POST['password'])) {
        ...
        if (strlen($_POST['password']) < 6) {
            ...
            $sql = "INSERT INTO users (full_name, email, telefone, documento, password, join_date, permissions) 
                    VALUES ('$name', '$email', '$telefone', '$documento', '$password', '$joinDate', '$role')";
            ...
            if ($insert) {
                ...
                $erro = "Erro ao adicionar novo usuário.";
            }
        }
    } else {
        $erro = "Todos os campos com asterisco são obrigatórios!";
    }
}
?>
        ...
        <div class="row">
            <div class="col-md-6" style="text-align: center; margin-left:25%">
                ...
                <form action="anfitriao.php" method="POST" class="form" id="add_user">
                    ...
                    <div class="col-sm-12">
                        <br>
                        <input style="background-color:#df8352; border: white; padding: 10px 20px;" 
                               type="submit" class="btn btn-info" 
                               name="<?= (isset($_GET['edit'])) ? 'edit' : 'add'; ?>" 
                               value="<?= (isset($_GET['edit'])) ? 'Editar usuário' : 'Cadastrar'; ?>">
